...
def _enrich_profile_summary_for_chat(user: dict, prof_summary: str) -> str:
    extras: list[str] = []
    if user.get("matched_method"):
        extras.append(f"Prior Method Match recommendation:\n{user['matched_method'][:1200]}")
    elif user.get("latest_recommendation"):
        extras.append(f"Prior provider recommendation:\n{user['latest_recommendation'][:1200]}")
    if user.get("selected_method"):
        extras.append(f"Client selected method: {user['selected_method']}")
    if user.get("prefer_not_to_use"):
        extras.append(f"Methods to avoid (stated preference): {user['prefer_not_to_use']}")
    if not extras:
        return prof_summary
    return f"{prof_summary}\n\n" + "\n\n".join(extras)


def _retrieval_country_scope(user: dict) -> str:
...
def generate_whatsapp_chat_reply(user: dict, incoming_msg: str) -> str:
    """General WhatsApp Q&A grounded in the user's profile when available."""
    language = user.get("language", "english")
    user_phone = user.get("phone", "unknown")
    has_profile = user_has_clinical_profile(user)

    if has_profile and is_vague_chat_question(incoming_msg):
        return trim_to_word_count(
            build_chat_clarification_reply(user, language),
            max_words=WHATSAPP_RECOMMENDATION_MAX_WORDS,
        )

    mec_text = "[User not yet registered for Method Match]"
    prof_summary = "[No clinical profile available]"
    if has_profile:
        stored_mec = user.get("latest_mec_text") or user.get("latest_mec_result")
        mec_text, prof_summary = _build_profile_context(user, language)
        if stored_mec and user.get("method_match_status") == "completed":
            mec_text = stored_mec
        prof_summary = _enrich_profile_summary_for_chat(user, prof_summary)
    ...
    chunks = retriever.retrieve(
        search_query,
        top_k=4,
        country_scope=_retrieval_country_scope(user),
    )
    context_str = retriever.format_context_for_llm(chunks)
    ...
    if has_profile:
        profile_instruction = (
            "\nThe user completed Method Match. Ground every answer in their profile "
            "(age, conditions, breastfeeding, preferences, prior recommendation). "
            "Do not contradict the WHO MEC section. If the question is ambiguous, "
            "ask ONE short clarifying question tied to their profile before advising."
        )
    ...
    reply_text = generate_gemini_text(
        f"{sys_prompt}{profile_instruction}\n\nUser Message: {incoming_msg}",
        max_output_tokens=WHATSAPP_MAX_OUTPUT_TOKENS,
        disable_thinking=True,
    )
...
def _trim_ussd_text(text: str) -> str:
    ussd_text = trim_to_word_count(text, max_words=35)
    if len(ussd_text) > 140:
        ussd_text = ussd_text[:137].rsplit(" ", 1)[0] + "..."
    return ussd_text


def generate_ussd_fast_mec_summary(user: dict) -> tuple[str, str]:
    """
    MEC-only USSD summary — no Gemini/RAG (typically <2s).
    Used when Redis is unavailable or as worker fallback.
    """
    language = user.get("language", "english")
    mec_text, _ = _build_profile_context(user, language)
    cards = build_fallback_method_cards(mec_text=mec_text, limit=3)
    avoid = _avoid_tokens_from_profile(user)
    filtered = []
    for card in cards:
        haystack = f"{card.get('name', '')} {card.get('category', '')}".lower()
        ...
        filtered.append(card)
    cards = (filtered or cards)[:3]
    names = [str(c.get("name") or "").strip() for c in cards if c.get("name")]
    prefixes = {
        "swahili": "Njia salama: ",
        "french": "Methodes: ",
        "portuguese": "Metodos: ",
    }
    prefix = prefixes.get(language, "Safe options: ")
    if not names:
        empty = {
            "swahili": "Hakuna njia salama sasa. Wasiliana na mhudumu wa afya.",
            "french": "Aucune methode sure maintenant. Consultez un agent de sante.",
            "portuguese": "Sem metodo seguro agora. Fale com um agente de saude.",
        }
        return _trim_ussd_text(empty.get(language, "No safe method now. Contact a health worker.")), mec_text
    return _trim_ussd_text(prefix + ", ".join(names)), mec_text
...
def generate_ussd_recommendation(user: dict) -> tuple[str, str]:
    ...
    return _trim_ussd_text(reply), mec_text
