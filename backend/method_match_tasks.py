...
from twilio_messaging import send_whatsapp_with_sms_fallback, send_whatsapp_message
...
def process_whatsapp_method_match_job(user_phone, to_number, lang, user_snapshot=None):
    ...
    try:
        ...
        delivery = send_whatsapp_with_sms_fallback(to_number, user_phone, reply_text)
        ...
        db.collection("contraceptive_users").document(user_phone).update({
            ...
            "method_match_delivery_channel": delivery.get("channel"),
            "method_match_delivery_status": delivery.get("status"),
            "method_match_delivery_error": delivery.get("error") or delivery.get("whatsapp_error") or "",
        })
        record_audit_event(
            db=db,
            phone=user_phone,
            ...
            metadata={
                "channel": delivery.get("channel") or "whatsapp",
                "method_count": len(method_cards or []),
            },
        )
